...
# .HUMAN_COMPS / .N_HUMAN_BLOCKS are defined in the  aaa_constants.R file


#' Build the simulation configuration object
#'
#' @param N total population size
#' @param P patch number @param A age groups
#' @param tf_days simulation horizon (days)
#' @param beta transmission rate
#' @param infectious_days,latent_days,immunity_days Mean times. 
#'   `Inf` for `immunity_days` to have (permanent immunity), and
#'   `Inf` for `infectious_days` to disable recovery (SI models).
#' @param p_asym asymptomatic fraction
#' @param asymp_trans asympotatic infectiousness
#' @param kappa asymptomatic to symptomatic rate
#' @param n_waning_stages 1, 2, or 3 sequential R compartments
#' @param no_latent no latent state
#' @param sis_mode  no recovery state
#' @param prep_start_rate S -> Sprep rate 
#' @param prep_active PrEP efficacy (susceptibility reduction) for individuals in `Sprep`. Co
#' @param prep_eff Deprecated and ignored. Efficacy was previously windowed,
#'   with `prep_eff` as the value outside the window; it is now constant at
#'   `prep_active`.
#' @param reactive epidemic threshold trigger
#'
#' @export
setup_sim <- function(
  ...
  # Contact reduc
  contact_mult = 1.0, contact_on_day = NULL, contact_off_day = NULL,
  ...
  prep_on_day = NULL, prep_off_day = NULL, prep_active = 0.7,
  ...
  # Contact reduction (school/work closure) eventual collapse into beta
  beta_vec <- beta_vec * seasonal_vec

  # Scheduled school / work closure. Reactive runs are gated 
  f_interv_vec <- rep(1.0, tf_days)
  if (!reactive && !is.null(contact_on_day) && contact_mult != 1.0) {
    c_on  <- max(1L, as.integer(contact_on_day))
    c_off <- if (!is.null(contact_off_day))
               min(as.integer(contact_off_day), tf_days) else tf_days
    if (c_off >= c_on) f_interv_vec[c_on:c_off] <- as.numeric(contact_mult)
  }
  ...
  list(
    ...
    # structural flags for the C++ ccghs
    no_latent = as.integer(isTRUE(no_latent)),
    sis_mode  = as.integer(isTRUE(sis_mode)),

    C = C, Mp = Mp, x0 = x0,
    # Needed downstream to size per-patch vector pools.
    patch_fracs = patch_fracs, pop_fracs = pop_fracs,

    beta_eff_daily = beta_vec,

    # Transmissions drivers to make it easier
    f_interv_daily        = f_interv_vec,
    f_contact_daily       = f_interv_vec,
    f_water_daily         = rep(1.0, tf_days),
    seasonal_daily        = rep(1.0, tf_days),
    ...
    # PreP campaign helper functions
    prep_start_daily      = if (length(prep_start_rate) > 1L)
                              expand(prep_start_rate, "prep_start_rate")
                            else
                              make_driver(0.0, prep_on_day, prep_off_day,
                                          prep_start_rate),
    ...
    # Vaccine / PrEP efficacy is a property of the protected individual,
    prep_eff_daily        = expand(prep_active, "prep_active"),
...
set_initial_conditions <- function(cfg,
                                   ...
                                   seed_mode        = "fraction",
                                   n_seed           = NA_integer_,
                                   seed_comp        = NA_character_,
                                   waning_stages    = 1L) {

  # Seeding should align with compartment sturcture
  if (is.null(has_quarantine)) has_quarantine <- has_intervention
  if (is.null(has_isolation))  has_isolation  <- has_intervention
  ...
  # Under SIS no R compartment
  if (isTRUE(cfg$sis_mode == 1L)) immune_frac <- 0.0

  n_R_blocks <- min(waning_stages, 3L)
  if (immune_frac > 0 && n_R_blocks >= 1) {
    # Allocate immunity in proportion to each stratum's own population
    for (p in 0:(P - 1)) for (a in 0:(A - 1)) {
      s_col  <- idx(0, p, a)
      N_cell <- 0
      for (blk in 0:11) N_cell <- N_cell + x0[idx(blk, p, a)]
      n_cell <- round(N_cell * immune_frac)
      if (n_cell <= 0) next
      per_R <- round(n_cell / n_R_blocks)
      for (r_blk in 9:(9 + n_R_blocks - 1)) {
        r_col    <- idx(r_blk, p, a)
        transfer <- min(per_R, x0[s_col])
  ...
  if (no_latent) {
    ...
    w_Ia  <- p_asym       * infectious_days
    w_Iso <- if (has_isolation)  w_Is * 0.10 else 0
    w_QL  <- if (has_quarantine) w_L  * 0.08 else 0
    weights <- c(L = w_L, Is = w_Is, Ia = w_Ia, Iso = w_Iso, QL = w_QL)
    blk_map <- c(L = 4L, Is = 6L, Ia = 7L, Iso = 8L, QL = 5L)
  }

  # Count mode: an absolute number of seeds, all in one compartment
  if (identical(seed_mode, "count") && !is.na(n_seed)) {
    n_total_seed <- max(1L, as.integer(n_seed))
    comp <- if (is.na(seed_comp)) "Is" else as.character(seed_comp)
    if (!comp %in% names(weights) || weights[[comp]] <= 0) comp <- "Is"
    weights <- stats::setNames(rep(0, length(weights)), names(weights))
    weights[[comp]] <- 1
    cfg$seed_comp_used <- comp
  }
