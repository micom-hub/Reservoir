...
#
# Applies overdispersed NegBin noise to raw case counts.
################################################################################
...
# apply_noise_limited
#
# Lightweight observation model: NegBin overdispersion only, no reporting
# pipeline.
################################################################################
...
#
# Four sequential stages, each enabled independently with ~80% probability:
...
#
# Lighter reporting pipeline used for hospitalizations (scale = 1.0) and
# deaths (scale = 0.75). Two optional stages: logistic underreporting ramp
# and weekend suppression.
################################################################################
...
# extract_symptomatic
#
# Extracts symptomatic case counts from total cases using p_asym.
# NegBin with large r (tight around mean) to avoid excess noise.
################################################################################
extract_symptomatic <- function(daily_cases, p_asym) {
  mu   <- daily_cases * (1 - p_asym)
  mask <- mu > 0
  symp <- numeric(length(mu))
  if (any(mask))
    symp[mask] <- rnbinom(sum(mask), size = 100,
                          prob = 100 / (100 + mu[mask]))
  as.integer(symp)
}
...
#   -NegBin hospitalizations with gamma admission lag
#   -NegBin deaths with CFR multiplier, temporal drift, optional spike,
#     optional bimodal lag distribution
#
...
generate_hosp_and_deaths <- function(daily_symp_by_age,
  ...
  # Population-level convolution — replaces per-patient rgamma loops.
  # Statistically equivalent for the daily signal; orders of magnitude faster.
  ...
  for (a in seq_len(A)) {
    ...
    # Hospitalizations: NegBin daily count → Gamma lag convolution
    ...
    # Deaths: fraction of hospitalisations → Gamma lag convolution
...
run_postprocessing <- function(result,
  ...
  # Aggregate daily cases across all (p,a) cells
