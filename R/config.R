...
# model_structure
#
# Declares which compartments and transitions exist
################################################################################
#' Declare the compartmental model structure
#'
#' @param has_latent Latent (exposed) stage present
#' @param has_asymptomatic Asymptomatic infectious class
#' @param has_recovery Recovery from infection
#' @param immunity "waning", "permanent", or "none"
...
#' @param has_pep Post-exposure prophylaxis, needs latent compartment
#' @param has_vaccination Leaky vaccination or PrEP via the Sprep compartment.
...
model_structure <- function(
    ...
    n_waning_stages  = c("1" = 0.6, "2" = 0.2, "3" = 0.2),
    has_isolation = TRUE,
    # intervention depends on intervention_policy and if the enable_prop is turned on
    has_quarantine = 0.5,
    has_pep = 0.3,
    has_vaccination = 0.2,
    has_demography = TRUE,
  ...
  # immunity error messages
  .imm_levels <- c("waning", "permanent", "none")
  if (is.character(immunity)) {
    immunity_probs <- stats::setNames(1, match.arg(immunity))
  } else if (is.numeric(immunity) && !is.null(names(immunity))) {
    if (!all(names(immunity) %in% .imm_levels))
      stop('`immunity` names must be among "waning", "permanent", "none"')
    if (anyNA(immunity) || any(immunity < 0))
      stop("`immunity` probabilities must be non-negative")
    if (abs(sum(immunity) - 1) > 1e-8)
      stop("`immunity` probabilities must sum to 1")
    immunity_probs <- immunity[immunity > 0]
  } else {
    stop('`immunity` must be "waning", "permanent" or "none", or a named ',
         "probability vector over them summing to 1")
  }
  immunity <- if (length(immunity_probs) == 1L)
                names(immunity_probs) else "<mixture>"

  # wanning stage error messages
  if (length(n_waning_stages) == 1L && is.null(names(n_waning_stages))) {
    if (!n_waning_stages %in% 1:3)
      stop("`n_waning_stages` must be 1, 2, or 3")
    nw_probs <- stats::setNames(1, as.character(as.integer(n_waning_stages)))
  } else {
    if (!is.numeric(n_waning_stages) || is.null(names(n_waning_stages)) ||
        !all(names(n_waning_stages) %in% c("1", "2", "3")))
      stop('`n_waning_stages` must be 1, 2 or 3, or a named probability ',
           'vector over c("1","2","3")')
    if (anyNA(n_waning_stages) || any(n_waning_stages < 0))
      stop("`n_waning_stages` probabilities must be non-negative")
    if (abs(sum(n_waning_stages) - 1) > 1e-8)
      stop("`n_waning_stages` probabilities must sum to 1")
    nw_probs <- n_waning_stages[n_waning_stages > 0]
  }
  n_waning_stages <- if (length(nw_probs) == 1L)
    as.integer(names(nw_probs)) else NA_integer_
  ...
  # Message checks for valid compartment mixtures
  # PEP acts on L and QL only message
  ...
  # immunity = "none" message
  if (identical(immunity, "none") && !identical(s$n_waning_stages, 1L) &&
      !is.na(s$n_waning_stages)) {
    warning('immunity = "none" (SIS) ignores `n_waning_stages`; forcing to 1')
    s$n_waning_stages <- 1L
    s$nw_probs        <- stats::setNames(1, "1")
  }

  # SI models have no recovery, message
  if (s$has_recovery == 0 && !identical(immunity, "permanent")) {
    warning("`has_recovery = FALSE` (SI model): immunity is undefined, ",
            'setting immunity = "permanent"')
    s$immunity       <- "permanent"
    s$immunity_probs <- stats::setNames(1, "permanent")
  }
...
print.reservoir_structure <- function(x, ...) {
  ...
  imm <- if (length(x$immunity_probs) > 1L)
           paste(sprintf("%s=%.2f", names(x$immunity_probs), x$immunity_probs),
                 collapse = " ")
         else x$immunity
  cat(sprintf("  immunity         : %s%s\n", imm,
              if ("none" %in% names(x$immunity_probs)) "   [STRUCTURAL]" else ""))
  nw <- if (length(x$nw_probs) > 1L)
          paste(sprintf("%s=%.2f", names(x$nw_probs), x$nw_probs), collapse = " ")
        else as.character(x$n_waning_stages)
  cat(sprintf("  n_waning_stages  : %s\n", nw))
...
################################################################################
# .skeleton_name: this is just the compartment name
################################################################################
...
#'
#' @param pop_size Total population N
#'   `pop_size_range`.
#' @param num_pops Number of patches P
#' @param num_ages Number of age groups A
#' @param num_days Simulation horizon`.
...
population_spec <- function(
    ...
    num_days = NULL,
    # Datasets use a fixed 5,000-day horizon. Still a probability vector, so a
    # user can restore a mixture by overriding it.
    num_days_probs = c("5000" = 1),
    ...
    # Seeding mode: fractions vs counts
    seed_mode = c("fraction", "count"),
    n_seed_range = c(1L, 50L),
    seed_comp_probs = c(Is = 0.6, L = 0.2, Ia = 0.2),
    novel = TRUE,
    immune_frac = NULL
) {
  ...
  structure(list(
    ...
    n_seed_range = as.integer(n_seed_range),
    seed_comp_probs = seed_comp_probs,
...
#' @param enable_prob probability that a given run has an intervention
#' @param start_day,duration Scheduled mode for interventions
#' @param on_threshold,off_threshold Reactive mode for interventions
#' @param trigger_delay Reactive mode for interventions that gives a delay
#' @param contact_reduction Reactive mode contact beta multiplier for work and school closures
#' @param iso_rate,quar_rate,pep_rate,prep_start_rate,prep_eff Intervention daily rates
...
intervention_policy <- function(
    ...
    #share of runs receiving any intervention
    enable_prob   = 0.25,
    ...
    # contact-reduction(folded into beta)
    school_closure  = NULL,
    work_closure = NULL
) {
  mode <- match.arg(mode)
  ...
  #messages when things for delay aren't specified
...
reservoir_config <- function(
    ...
    r0_base = list(shape = 1.2,  rate = 1.00, shift = 0.8, floor = 0.3),
    r0_vector  = list(shape = 3.2,  rate = 0.90, shift = 0.4, floor = 0.3),
    r0_waterborne = list(shape = 2.6,  rate = 0.85, shift = 0.4, floor = 0.3),

    # epidemiological params
    infectious_days = list(meanlog = log(5), sdlog = 0.5, lo = 1,    hi = 30),
    latent_days     = list(meanlog = log(5), sdlog = 0.6, lo = 0.5,  hi = 21),
    immunity_days   = list(meanlog = log(365), sdlog = 1.0, lo = 14, hi = 10950),
    p_asym_beta = c(3, 7),
    ...
    # demographics and importation
    birth_rate_range = c(2e-5, 1.2e-4),
    death_rate_factor = c(0.8, 1.3),
    importation_logrange = c(log(0.01), log(0.5)),
    ...
    # clinical severity
    hosp_prob_range  = c(0.005, 0.15),
    death_prob_range = c(0.001, 0.10),
    ...
    vector = list(
      ...
      b_h_range = c(0.1, 0.8),
      b_v_range = c(0.1, 0.8),
      # Dispersion of age-specific biting exposure
      bite_age_sdlog = 0.3,
      Nv_init_frac_range = c(0.05, 2.0),
      Iv_frac_range = c(0.001, 0.05)
    ),
    waterborne = list(
      eta_I_logrange = c(log(0.1),  log(1.0)),
      eta_A_logrange = c(log(0.05), log(0.5)),
      alpha_env_range = c(0.1, 1.0),
      mu_w_logrange = c(log(0.01), log(0.5)),
      waterborne_frac_range = c(0.1, 0.9),
      w_unit = 1.0, #units for W
      delta_eff_bounds = c(1e-14, 1.0)
    ),

    # Retention; detrermines which runs are kept/discarded
    min_cases_endemic    = 5L,
    min_cases_nonendemic = 10L,
...
print.reservoir_config <- function(x, ...) {
  ...
  cat(sprintf("  R0 (base)     : LogNormal(%.2f, %.2f) + %.1f\n",
              x$r0_base$shape %||% exp(x$r0_base$meanlog),
              x$r0_base$rate  %||% x$r0_base$sdlog, x$r0_base$shift))
...
################################################################################
# .scope_config - what signals are gnerated per run
#   "full"       everything
...
################################################################################
# .sample_structure - draw concrete structure
################################################################################
.sample_structure <- function(s) {
  out <- list(
    ...
    immunity = local({
      pr <- s$immunity_probs %||% stats::setNames(1, s$immunity)
      if (length(pr) == 1L) names(pr) else .sample1(names(pr), prob = pr)
    }),
    n_waning_stages  = local({
      pr <- s$nw_probs %||% stats::setNames(1, as.character(s$n_waning_stages))
      if (length(pr) == 1L) as.integer(names(pr))
      else as.integer(.sample1(names(pr), prob = pr))
    }),
  ...
  # SI models have no recovery
  if (!out$has_recovery) out$immunity <- "permanent"
