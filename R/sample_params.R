...
# sample_params.R
#
# Hierarchical parameter samplers for all three transmission mechanisms.
#
################################################################################
...
compute_ip <- function(infectious_days, latent_days, p_asym,
  ...
  # time transmitting in Ia + P(Ia -> Is) * time transmitting in Is
  ...
  if (has_latent) {
    ...
    # No latent stage: branch directly at infection
...
sample_population_structure <- function(population) {
  ...
  # Age multipliers for hospitalzsation / death; increase with age
...
# sample_shared_epi
#
# Structure flags override sampled values:
#   has_asymptomatic = FALSE  -> p_asym = 0 (Ia channel zero-propensity)
#   immunity = "permanent" -> omega = 0 (R absorbing)
#   immunity = "none" -> SIS handled structurally; omega = 0
#   has_recovery = FALSE -> gamma = 0 (SI); beta must be supplied directly
#   has_latent = FALSE -> latent_days irrelevant (engine bypasses L)
################################################################################
...
sample_scenario <- function(beta, structure, population, policy, config) {
  ...
  if (n_waves > 0 && tf_days > 60) {
    wave_change_days <- sort(sample(seq(50L, max(51L, tf_days - 10L)), n_waves))
  ...
  # Day-to-day environmental stochasticity on beta. Deliberately independent
  # of seasonality: real epidemic curves are rough at large N because
  # transmission fluctuates (weather, behaviour, events), not because of
  # demographic noise, and that roughness is present with or without a
  # seasonal cycle.
  ...
  if (has_intervention) {
    ...
    school_red <- policy$school_closure %||% 0
    work_red   <- policy$work_closure   %||% 0
  ...
  hosp_prob <- .logunif(config$hosp_prob_logrange)
  death_prob <- .logunif(config$death_prob_logrange)
  ...
  list(
    ...
    seed_frac = seed_frac, novel = population$novel,
...
sample_params_base <- function(structure, population, policy, config) {
  p  <- config$r0_base
  R0 <- max(rlnorm(1, p$meanlog, p$sdlog) + p$shift, p$floor)
  ...
  if (structure$has_recovery) {
    ...
    beta <- R0/max(ip * struct$structural_mult, 1e-8)
    beta <- pmax(config$beta_bounds[1], pmin(beta, config$beta_bounds[2]))
  } else {
    # SI / SLI: IP diverges as gamma -> 0, so the R0 inversion is undefined.
...
sample_params_vector <- function(structure, population, policy, config) {
  ...
  if (structure$has_recovery) {
    ip <- compute_ip(epi$infectious_days, epi$latent_days, epi$p_asym,
                     epi$asymp_trans, epi$kappa, structure$has_latent)
    # The engine drives BOTH direct human-to-human transmission and the
    # vector cycle from the same beta, so the inversion must account for both.
    #   k1 = human->human next-generation factor (uses the contact structure)
    #   k2 = vector cycle factor (vector pool is unstructured: no structural_mult)
    # rho solves rho^2 - beta*k1*rho - beta^2*k2 = 0, giving
    #   R0 = beta * (k1 + sqrt(k1^2 + 4*k2)) / 2
    time_Iv <- (sigma_v / max(sigma_v + mu_v, 1e-8)) * (1 / max(mu_v, 1e-8))
    k1 <- ip * struct$structural_mult
    k2 <- a_bite * a_bite * b_h * b_v * time_Iv * ip
    beta <- 2 * R0 / max(k1 + sqrt(k1 * k1 + 4 * k2), 1e-8)
    beta <- pmax(config$beta_bounds[1], pmin(beta, config$beta_bounds[2]))

    # record the route split for diagnostics
    R0_h2h    <- beta * k1
    R0_vector <- R0 - R0_h2h
  } else {
    if (is.null(config$beta_direct))
...
sample_params_waterborne <- function(structure, population, policy, config) {
  ...
  # delta_eff depends on N, so resolve the population size first and pass the
  # same value into sample_scenario() so the two cannot diverge.
  ...
  if (structure$has_recovery) {
    ...
    beta_contact <- R0_contact / max(ip * struct$structural_mult, 1e-8)
    beta_contact <- pmax(config$beta_bounds[1],
                         pmin(beta_contact, config$beta_bounds[2]))

    R0_water <- R0 * waterborne_frac
    shed_rate <- eta_I * (1 - epi$p_asym) + eta_A * alpha_env * epi$p_asym
    # W accumulates ABSOLUTE shedding, so mean_W scales with the number of
    # infected people. Dividing by N makes the water route frequency-dependent,
    # matching the contact route and keeping R0 independent of population size.
    mean_W <- shed_rate * ip / max(mu_w, 1e-8)
    delta_eff <- R0_water / max(mean_W * N_pop, 1e-8)
    delta_eff <- pmax(w$delta_eff_bounds[1], pmin(delta_eff, w$delta_eff_bounds[2]))
  } else {
    if (is.null(config$beta_direct))
      stop("`has_recovery = FALSE` requires `beta_direct` in reservoir_config()")
...
# DIAGNOSTICS
################################################################################
#' Summarise the marginal distributions produced by the sampler
#'
#' @export
