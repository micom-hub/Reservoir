...
.run_layout <- function(run, fallback_mech = "base") {
  ...
  # Non-human compartments occupy blocks of `extra_len` columns each: 1 for the
  # environmental reservoir (a single scalar), P for the vector pools (one per
  # patch). Falls back to 1 for cfgs written before per-patch vector pools.
  extra_len <- cfg$vector_block_len %||% 1L
  list(comp_names = comp_names, n_human = n_human, extra_len = extra_len,
...
.run_to_df <- function(r, fallback_mech, aggregate, include_noise) {
  ...
  for (ci in seq_along(lay$comp_names)) {
    ...
    if (ci <= lay$n_human) {
      ...
      # Non-human blocks appended after the human ones, `extra_len` columns
      # each. Vector pools are per-patch (extra_len = P); the environmental
      # reservoir is a single scalar (extra_len = 1).
      elen  <- lay$extra_len
      start_col <- M_human + (ci - lay$n_human - 1L) * elen + 1L
      end_col   <- start_col + elen - 1L
      ...
      if (end_col > ncol(sot)) {
        ...
        df[[nm]] <- rowSums(sot[, start_col:end_col, drop = FALSE])
      } else {
        for (p in 0:(elen - 1L))
          df[[sprintf("%s_p%d", nm, p)]] <- sot[, start_col + p]
      }
    }
  }
...
extract_all_runs_single_csv <- function(rds_path, out_path,
  ...
  # for all the columns eventual bind all
