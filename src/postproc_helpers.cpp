...
NumericVector apply_reporting_delays_cpp(NumericVector cases,
  ...
  // Pre-compute the unnormalised Gamma kernel once, out to hard_max.
  ...
  for (int t = 0; t < n; ++t) {
    ...
    // Normalise the kernel over the active window.
    double ksum = 0.0;
...
IntegerVector hospital_occupancy_cpp(IntegerVector daily_hosp, int avg_los) {
  ...
  // Above this many admissions in a day, sample a mean LOS for the whole
  // batch instead of per-individual
