...
//
// W: 1 continuous scalar appended at end (index M_human)
...
//   Infection: 2  (S->L, Sprep->L)
//   PrEP: 2  (S->Sprep, Sprep->S)
//   Quarantine: 5  (S->Qs, Qs->S, Qs->Sprep, L->QL, Sprep->Qs)
//   PEP/Spost: 3  (L->Spost, QL->Spost, Spost->S)
//   L progression: 2  (L->Is, L->Ia)
//   QL progression: 2  (QL->Ia, QL->Iso)
//   Asymp transitions: 3  (Ia->Is, Ia->Iso, Ia->R)
//   Symp transitions: 2  (Is->Iso, Is->R)
//   Iso recovery: 1  (Iso->R)
//   Waning: 3  (R->R2, R2->R3, R3->S)
//   Birth: 1
//   Deaths: 12  (S Sprep Spost Qs L QL Is Ia Iso R R2 R3)
//
// W is continuous - updated analytically each tau step, no discrete reactions.
...
static std::vector<Reaction> build_reactions(
  ...
  for (int p = 0; p < P; ++p) {
    for (int a = 0; a < A; ++a) {
      ...
      // Waning immunity (three-stage, matches human2human)
      add({iR,     iR2},    {-1, +1});        // 23 R     -> R2
      add({iR2,    iR3},    {-1, +1});        // 24 R2    -> R3
      add({iR3,    iS},     {-1, +1});        // 25 R3    -> S
...
// update_W
//
// W is a continuous pathogen concentration - not a discrete count.
// Integrated exactly over dt via closed-form ODE solution:
//
...
static double update_W(
  ...
  double input = eta_I * Is_total + eta_A * alpha_env * Ia_total;
  double W_new;
  if (mu_w > 1e-12) {
    double decay = std::exp(-mu_w * dt);
    W_new = W * decay + (input / mu_w) * (1.0 - decay);
  } else {
    W_new = W + input * dt;
  }
  return std::max(0.0, W_new);
}
...
//
// 38 propensities per (p,a), matching build_reactions exactly
static void compute_propensities(
  ...
  for (int p = 0; p < P; ++p) {
    for (int aa = 0; aa < A; ++aa) {
      ...
      a[j++] = omega * R;                              // 23 R     -> R2
      a[j++] = omega * R2;                             // 24 R2    -> R3
      a[j++] = omega * R3;                             // 25 R3    -> S
      ...
      a[j++] = death_rate * R;                         // 36
      a[j++] = death_rate * R2;                        // 37
      a[j++] = death_rate * R3;                        // 38
    }
  }
...
List simulate_env_atl_cpp(List cfg) {
  ...
  // ---- state vector: 13*P*A human entries + 1 W slot ----
  int block   = P * A;
  ...
  // W excluded from tau selection - updated deterministically
  ...
  for (int day = 0; day < tf_days; ++day) {
    ...
    while (t_in_day < 1.0) {
      ...
      if (tau < exactThreshold / totalRate) {
        ...
        if (which >= 0) {
          ...
          for (size_t k = 0; k < r.idx.size(); ++k)
            ...
            if (ok) {
              apply_reaction(x, r);
              x[iW] = update_W(x[iW], sum_block_fn(offIs), sum_block_fn(offIa),
                               eta_I, eta_A, alpha_env, mu_w, dt);
            }
        }
        t_in_day += dt;
        continue;
        ...
        if (which >= 0) {
          ...
          for (size_t k = 0; k < r.idx.size(); ++k)
            ...
            if (ok) {
              apply_reaction(x, r);
              x[iW] = update_W(x[iW], sum_block_fn(offIs), sum_block_fn(offIa),
                               eta_I, eta_A, alpha_env, mu_w, dt2);
            }
        }
        t_in_day += dt2;
        continue;
      ...
      x[iW] = update_W(W0, sum_block_fn(offIs), sum_block_fn(offIa),
                       eta_I, eta_A, alpha_env, mu_w, dt);
      t_in_day += dt;
