...
// RXN count per (p,a):
//   Base (no extra waning stages): 30 reactions
//   +1 for each extra waning stage beyond 1 (i.e. R->R2, R2->R3)
//   Deaths: 10 compartments always (S,Sprep,Spost,Qs,L,QL,Is,Ia,Iso,R)
//         + 1 per extra waning stage (R2, R3)
static std::vector<Reaction> build_reactions(
  ...
  for (int p = 0; p < P; ++p) {
    for (int a = 0; a < A; ++a) {
      ...
      // Spost -> S (PEP protection wanes) -- always present
      add({iSpost, iS}, {-1, +1});
...
// Iso does NOT contribute (isolated = no contacts)
...
List simulate_structured_atl_cpp(List cfg) {
  ...
  // offsets
  // Blocks: S(0) Sprep(1) Spost(2) Qs(3) L(4) QL(5) Is(6) Ia(7) Iso(8)
  //         R(9) R2(10) R3(11) cumul(12)
  // R2 and R3 slots always allocated in state vector for simplicity,
  // but only used if n_waning_stages >= 2 or 3.
  int block = P * A;
  if (M != block * N_BLOCKS)
  ...
  for (int day = 0; day < tf_days; ++day) {
    ...
    // Uses the PREVIOUS day's observed new cases as the trigger signal, so the
    // decision for day d depends only on information available at end of d-1.
    if (reactive_mode) {
      if (!interv_active) {
